...
def vlm_inference(text=None, images=None, sys_message=None, processor=None, model=None, tokenizer=None, args=None):
        if "llama" in args.model_path or "Llama" in args.model_path:
            ...
            text = processor.apply_chat_template(
                message, tokenize=False, add_
### **Explanation of Current Intent:**

1. **Traffic Behavior:**  
   - The vehicle ahead continues traveling in the same lane at a consistent speed. There are no signs of braking or sudden movements that would require changes in my intent.  
   - Traffic in adjacent lanes—primarily to the right side—remains steady with no vehicles attempting to merge or behave erratically.

2. **Lane and Road Conditions:**  
   - The lane markings are clearly visible and unobstructed, guiding the vehicle down a safe, continuous path.  
   - The lane I am traveling in remains open, with no debris, construction cones, or sudden narrowing detected. The road ahead maintains good clarity, and there are no visible obstructions or hazards requiring a change in intent.

3. **Environmental Factors:**  
   - Lighting conditions remain consistent, offering sufficient visibility to evaluate the roadway and traffic.  
   - There are no adverse weather conditions such as rain or fog hindering the view or road surface integrity.

4. **Safety Considerations:**  
   - A safe following distance is maintained from the vehicle in front, ensuring enough reaction time in case of a sudden stop or hazard.  
   - No pedestrians, cyclists, or other vulnerable road users are visible in the lane or on the immediate roadside, ruling out the need for any immediate deceleration or swerving.  
   - The adjacent lane to the left remains visibly clear, should an emergency lane change become necessary, but no such action is currently required.

---

### **Final Decision:**  
There are no changes in the driving environment that require any modification to my current driving intent. Based on the consistent movement of vehicles, unobstructed lane markings, and safe conditions, I will continue driving straight and maintaining my current speed.
Observed Speed and Curvature: [5.8,0.1], [6.6
### **Explanation of Current Intent:**
...
### **Final Decision:**  
There are no changes in the driving environment that require any modification to my current driving intent. Based on the consistent movement of vehicles, unobstructed lane markings, and safe conditions, I will continue driving straight and maintaining my current speed.
Observed Speed and Curvature: [5.8,0.1], [6.6,0.0], [6.8,-0.0], [6.9,0.0], [7.0,0.2], [7.1,0.1], [7.8,0.1], [7.3,0.1], [6.7,0.1], [7.5,0.2]
HTTP Request: POST https://api.openai.com/v1/chat/completions "HTTP/1.1 200 OK"
Got 10 future actions: [[7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1]]
(openemma) cralab@bizon:~/Projects/av_vla$ ,0.0], [6.8,-0.0], [6.9,0.0], [7.0,0.2], [7.1,0.1], [7.8,0.1], [7.3,0.1], [6.7,0.1], [7.5,0.2]
HTTP Request: POST https://api.openai.com/v1/chat/completions "HTTP/1.1 200 OK"
Got 10 future actions: [[7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1], [7.4, 0.1]]
(openemma) cralab@bizon:~/Projects/av_vla$ generation_prompt=True
            )
